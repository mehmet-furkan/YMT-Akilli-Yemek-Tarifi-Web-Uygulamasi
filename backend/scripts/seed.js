/**
 * Seed Script — Night Code Kitchen
 *
 * Veritabanını örnek verilerle doldurur: 5 kullanıcı, 30 tarif, 10 favori.
 * Çalıştırmadan önce backend/.env içinde MONGO_URI tanımlı olmalı.
 *
 * Kullanım:  npm run seed   (backend/ klasöründen)
 *
 * Orijinal içerik: Emine Zehra Duyar (gorev-1.6).
 * JS'e çevirme + main şemasına uyarlama: PM entegrasyonu.
 */
...
const URI = process.env.MONGO_URI || "mongodb://localhost:27017/nightcodekitchen";

// Kısa yardımcı fonksiyonlar
const ing = (name, amount, unit = "") => ({ name, amount, unit });
const rec = (
  title, description, category, difficulty,
  cookTime, prepTime, servings, tags,
  ingredients, instructions, createdBy
...
async function seed() {
  ...
  // 5 Kullanici (preferences.diet main User.js semasina gore)
  const users = await User.create([
    { name: "Zehra Kaya",   email: "zehra@test.com", password: "Test1234", preferences: { diet: ["Vejetaryen"] } },
    { name: "Ahmet Yılmaz", email: "ahmet@test.com", password: "Test1234", preferences: { diet: [] } },
    { name: "Fatma Demir",  email: "fatma@test.com", password: "Test1234", preferences: { diet: ["Vegan"] } },
    { name: "Mehmet Şahin", email: "mehmet@test.com", password: "Test1234", preferences: { diet: ["Glutensiz"] } },
    { name: "Ayşe Çelik",   email: "ayse@test.com",  password: "Test1234", preferences: { diet: ["Laktozsuz"] } },
  ]);
  ...
  const recipes = await Recipe.create([
    ...
    // --- SALATA (4) ---
    rec("Çoban Salatası", "Taze sebze salatası.", "Salata", "Kolay", 1, 10, 4,
      ["taze", "salata"],
    ...
    rec("Semizotu Salatası", "Yoğurtlu semizotu.", "Salata", "Kolay", 1, 10, 2,
      ["yoğurt", "ot"],
    ...
    rec("Mevsim Salatası", "Karışık yeşil salata.", "Salata", "Kolay", 1, 10, 4,
      ["yeşil", "mevsim"],
    ...
    rec("Aşure", "40 malzemenin buluşması.", "Tatlı", "Orta", 120, 480, 12,
      ["geleneksel", "buğday"],
      [ing("aşurelik buğday","1","su bardağı"), ing("nohut","0.5","su bardağı"), ing("kuru üzüm","3","yemek kaşığı"), ing("şeker","1","su bardağı")],
      ["Buğday ve nohutları haşla.", "Şeker ve kuruyemişleri ekle.", "Kaselere dök, cevizle süsle."], u0),

    // --- İÇECEK (2) ---
    rec("Ayran", "Soğuk tuzlu yoğurt içeceği.", "İçecek", "Kolay", 1, 5, 4,
      ["yoğurt", "serin"],
  ...
  console.log(`${recipes.length} tarif, ${users.length} kullanıcı, 10 favori eklendi`);
